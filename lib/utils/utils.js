...
export function processComponents(components) {
	return components.map(function(component) {
		...
		if (component.props.example) {
			component.props.examples = [].concat(
...
export function filterComponentsInSectionsByExactName(sections, name) {
	...
	sections.forEach(function(section) {
		if (section.components) {
			components.push(..._toConsumableArray(filterComponentsByExactName(section.components, name)));
		}
		if (section.sections) {
			components.push(
				..._toConsumableArray(filterComponentsInSectionsByExactName(section.sections, name))
			);
		}
	});
	return components;
...
export function findSection(sections, name) {
	...
	try {
		for (
			var _iterator = sections[Symbol.iterator](), _step;
			...
			const _found = findSection(section.sections, name);
			if (_found) {
				return _found;
			}
		}
...
export function getInfoFromHash() {
	const hash =
		arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : window.location.hash;
...
export function getUrl() {
	...
	let _ref3 = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : window.location,
		origin = _ref3.origin,
