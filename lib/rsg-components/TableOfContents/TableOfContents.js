...
const TableOfContents = (function(_Component) {
	...
	function TableOfContents() {
		...
		return (
			(_ret = ((_temp = ((_this = _possibleConstructorReturn(
				this,
				(_ref = TableOfContents.__proto__ || Object.getPrototypeOf(TableOfContents)).call.apply(
					_ref,
					[this].concat(args)
				)
			)),
	...
	_createClass(TableOfContents, [
		{
			key: 'renderLevel',
			value: function renderLevel(pages) {
				const _this2 = this;

				const items = pages.map(function(page) {
					const children = [].concat(
						_toConsumableArray(page || []),
						_toConsumableArray(page.components || [])
					);
					const retVal = Object.assign({}, page, {
				...
				return React.createElement(ComponentsList, { items });
			},
		},
		{
			key: 'renderSections',
				...
				return React.createElement(
					TableOfContentsRenderer,
					{
						...
						onSearchTermChange: function onSearchTermChange(searchTerm) {
							return _this3.setState({ searchTerm });
						},
					},
					this.renderSections()
				);
