...
const StyleGuide = (function(_Component) {
	...
	function StyleGuide() {
		...
		return _possibleConstructorReturn(
			this,
			(StyleGuide.__proto__ || Object.getPrototypeOf(StyleGuide)).apply(this, arguments)
		);
	}
	...
	_createClass(StyleGuide, [
		...
		},
		{
			...
			value: function render() {
				...
				if (welcomeScreen) {
					return React.createElement(Welcome, { patterns });
				}
				...
				var sectionsToRender = sections.map(function(page) {
					if (document.location.hash.substr(1, document.location.hash.length - 1) === page.slug) {
						const arr = [page];
						return (sectionsToRender = React.createElement(Sections, {
							sections: arr,
							root: true,
						}));
					}
				...
				if (allNull) {
					...
					sectionsToRender = React.createElement(Sections, { sections: arr, root: true });
				}
				...
				return React.createElement(
					StyleGuideRenderer,
					{
						...
						toc: React.createElement(TableOfContents, { sections }),
